                ...
                <div class="mb-6">
                    ...
                    <div class="bg-gray-50 rounded-lg p-6">
                        <div class="flex items-start space-x-4">
                            <!-- Student Photo -->
                            <div class="flex-shrink-0 relative group cursor-pointer" onclick="document.getElementById('profile-pic-upload').click()">
                                <?php if($enrollment && $enrollment->photo): ?>
                                    <img src="<?php echo e(asset('storage/' . $enrollment->photo)); ?>" alt="Profile" class="w-20 h-20 rounded-full object-cover border-4 border-white shadow-md hover:opacity-80 transition-opacity">
                                <?php elseif($student && $student->photo): ?>
                                    <img src="<?php echo e(asset('storage/' . $student->photo)); ?>" alt="Profile" class="w-20 h-20 rounded-full object-cover border-4 border-white shadow-md hover:opacity-80 transition-opacity">
                                <?php else: ?>
                                    <div class="w-20 h-20 rounded-full border-4 border-white shadow-md bg-gradient-to-r from-blue-600 to-blue-400 flex items-center justify-center hover:opacity-80 transition-opacity">
                                ...
                                <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 rounded-full flex items-center justify-center transition-all duration-200">
                                    <i class="fas fa-camera text-white text-lg opacity-0 group-hover:opacity-100 transition-opacity"></i>
                                </div>
                            </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-3">
                            ...
                            <li class="flex items-center">
                                ...
                                <span class="text-gray-600">Total Payment: <span class="font-medium">₱<?php echo e(number_format($student->payments()->where('status', 'completed')->sum('amount') ?? 0, 2)); ?></span></span>
                            </li>
                            <li class="flex items-center">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle file input change for profile photo upload
            ...
            reader.onload = function(event) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (profileImg) {
                            profileImg.src = '<?php echo e(($enrollment && $enrollment->photo) ? asset("storage/" . $enrollment->photo) : (($student && $student->photo) ? asset("storage/" . $student->photo) : "")); ?>';
                        }
                    }
                })
                .catch(error => {
