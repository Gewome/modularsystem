                        ...
                        <div class="flex items-start space-x-4">
                            ...
                            <div class="flex-shrink-0 relative group cursor-pointer" onclick="document.getElementById('profile-pic-upload').click()">
                                @if($enrollment && $enrollment->photo)
                                    <img src="{{ asset('storage/' . $enrollment->photo) }}" alt="Profile" class="w-20 h-20 rounded-full object-cover border-4 border-white shadow-md hover:opacity-80 transition-opacity">
                                @elseif($student && $student->photo)
                                    <img src="{{ asset('storage/' . $student->photo) }}" alt="Profile" class="w-20 h-20 rounded-full object-cover border-4 border-white shadow-md hover:opacity-80 transition-opacity">
                                @else
                    ...
                    <div>
                        ...
                        <ul class="space-y-3">
                            ...
                            <li class="flex items-center">
                                ...
                                <span class="text-gray-600">Total Payment: <span class="font-medium">₱{{ number_format($student->payments()->where('status', 'completed')->sum('amount') ?? 0, 2) }}</span></span>
                            </li>
                            <li class="flex items-center">
            ...
            reader.onload = function(event) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (profileImg) {
                            profileImg.src = '{{ ($enrollment && $enrollment->photo) ? asset("storage/" . $enrollment->photo) : (($student && $student->photo) ? asset("storage/" . $student->photo) : "") }}';
                        }
                    }
                })
                .catch(error => {
